...
if (isNull _nearestEnemy) then {
	_nearestEnemy = _leader call FUNC(ClstEmy);	
};
...
//Now tell the AI to seek cover.
{
	...
	if (_Foot) then	{
		[_x,_typeListFinal,_weakListFinal,_nearestEnemy,_moveDist,_movePosition] spawn {
			...
			if (count _concealL > 0) then {_posCON = selectRandom _concealL;}; //[_concealL,_unit,true,"NrstPos"] call keko_vcomai_fnc_ClstObj;
			if (count _coverL > 0) then {_posCON = selectRandom _coverL }; //_posCON = [_coverL,_unit,true,"NrstPos"] call keko_vcomai_fnc_ClstObj;
				
				//Return fire for a few seconds, then move
				private _fNearestEnemy = _unit findNearestEnemy _unit;
				if (isNull _fNearestEnemy) then
				{
					_fNearestEnemy = _unit call FUNC(ClstEmy);	
				};
				...
				private _Cnt = 0;
				while {_Cont} do {
					if (_unit distance2D _FPos < _DistW) then {
						_Cont = false;
					};
					if (!(alive _unit) || _Cnt > 80) then {
						_Cont = false;
						_Fail = true;
					};
					_Cnt = _Cnt + 0.1;	
