...
/*
	...
	Example1: groupAlpha call keko_vcomai_fnc_ArmStatics;
*/
...
{
	private _unit = [_unitArray,_x,true,"W1"] call FUNC(ClstObj);

	// keko_vcomai_fnc_ClstObj returns [0,0,0] if nothing found
	...
	_x spawn {
		...
		if (isNull _assignedGunner) then {
			...
			[_unit,_weap] spawn {
				...
				private _statictime = GVAR(STATICARMT);
