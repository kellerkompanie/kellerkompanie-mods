...
if (_grp getVariable [QGVAR(NOFLANK),false]) exitWith {};
...
private _AssignedWaypoints = _EnemyGroup getvariable [QGVAR(WAYPOINTS),[]];
...
_EnemyGroup setVariable [QGVAR(WAYPOINTS),_AssignedWaypoints,true];
...
switch (_wayPointType) do {
    ...
    case "High": {
		private _highP = ([_leader,500,50,false] call FUNC(Heights)) select 0 select 1;
    ...
    case "Low": {
		private _highP = ([_leader,500,50,true] call FUNC(Heights)) select 0 select 1;
		private _finalP = [[[_highP, 50]],["water"]] call BIS_fnc_randomPos;
		_finalP set [2,0];
		private _waypoint0 = _grp addwaypoint [_finalP,0];
		_waypoint0 setwaypointtype "MOVE";
