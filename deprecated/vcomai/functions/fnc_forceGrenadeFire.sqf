...
private _NEnemies = (leader _unit) call FUNC(ClstKnwnEnmy);
...
if (count _NEnemies > 0) then {
	...
	if (count _TargetArray > 0 && {(_Grenade || _Override)}) then {
			private _NE = [_TargetArray,_Unit,true,""] call FUNC(ClstObj);
			...
			if (_NE distance2D _Unit < 100) then {
				{
					if (!(["smoke",_x] call BIS_fnc_inString) && !(["IR",_x] call BIS_fnc_inString)) then {
						...
						if !(_FinalGrenade isEqualTo "none") then {
							...
							sleep 1;
							_unit setVariable [QGVAR(muzzle),_x];
							_unit setVariable [QGVAR(NE),_NE];
							_TempID = _unit addEventHandler ["Fired", 
							{
								params ["_unit", "_weapon", "_muzzle", "_mode", "_ammo", "_magazine", "_projectile", "_gunner"];
								private _getMuzzle = _unit getVariable [QGVAR(muzzle),"none"];
								private _NE = _unit getVariable [QGVAR(NE),"none"];
