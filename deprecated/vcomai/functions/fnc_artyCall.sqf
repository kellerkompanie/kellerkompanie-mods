...
} foreach GVAR(ARTYLST);

{
		private _Num = _x;
		private _Index = GVAR(ARTYLST) findIf {_x isEqualTo _Num};
		GVAR(ARTYLST) deleteAt _Index;
} foreach _RemoveList;

//Let's find an artillery group that can provide support.
private _artyArray = [];
{
	if ([(side _x),_CallSide] call BIS_fnc_sideIsFriendly && {side _x in GVAR(ARTYSIDES)}) then	{
...
private _clstGrp = [_artyArray, (leader _callGrp), true, "Arty1"] call FUNC(ClstObj);
		...
		if !(isNil "_ammoArray") then {
			{
				...
				if (["AT", _Type] call BIS_fnc_inString || ["Guided", _Type] call BIS_fnc_inString) then {
...
if (isNil "_randomAmmoArray") exitWith {
	if (GVAR(Debug)) then {
		[(leader _clstGrp),"NO AMMO"] call FUNC(DebugText);
	};
};
...
if !(_continueFiring) exitWith {
	if (GVAR(Debug)) then {
		[(leader _clstGrp),"NOT APPROPRIATE RANGE"] call FUNC(DebugText);
	};
};


//Make sure we do minimal friendly fire.
private _FriendlyArray = (leader _clstGrp) call FUNC(FriendlyArray);
...
if (_RndNumber isEqualTo 1) then {
	...
	private _CF = [_FriendlyArray,_positions,true,"Arty1"] call FUNC(ClstObj);
	if (_CF distance2D _positions > 50) then {
		_FireUnit doArtilleryFire [_positions,_randomAmmoArray,_RndNumber];	
		if (GVAR(Debug)) then {
			[(leader _clstGrp),"DO FIRE"] call FUNC(DebugText);
		};
	}
	else {
		if (GVAR(Debug)) then {
			[(leader _clstGrp),"SMOKE FIRE"] call FUNC(DebugText);
		};
...
else {
	{
		...
		private _CF = [_FriendlyArray,_positions,true,"Arty1"] call FUNC(ClstObj);
		if (GVAR(Debug)) then {
			[(leader _clstGrp),"DO FIRE 2"] call FUNC(DebugText);
		};
		if (_CF distance2D _positions > 50) then {
