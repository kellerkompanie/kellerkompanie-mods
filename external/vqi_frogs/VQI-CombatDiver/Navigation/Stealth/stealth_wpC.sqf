//
//
if (VQI_FROGS_HINTS == 1) then { hint "stealth_wpC"; };
...
while {subNav} do {
	...
	onMapSingleClick "posSUBCs = _pos; MapClickTravel = true; onMapSingleClick ''; true";
	waitUntil {MapClickTravel}; publicVariable "posSUBCs"; sleep 1;
	...
	// Max Speed
	_wp = posSUBCs distance2D posSUBBs;
	
	
	if (_wp < 1000) then { 
		
		// Min Depth
		_depth = getTerrainHeightASL posSUBCs;
		if (_depth < -30) then { 
			subNav = false; 
		} else { 
...
_ping1 = nearestObjects [posSUBCs, ["VQI_SUB_Class1"], 2000]; sleep 1;		// ENEMY SONAR RANGE - 2nd PING - TORP IN WATER!
_ping2 = nearestObjects [posSUBCs, ["VQI_SUB_Class2"], 3000]; sleep 1;		// ENEMY SONAR RANGE - 2nd PING - TORP IN WATER!
_ping3 = nearestObjects [posSUBCs, ["VQI_SUB_Class3"], 4000]; sleep 1;		// ENEMY SONAR RANGE - 2nd PING - TORP IN WATER!
_ping4 = nearestObjects [posSUBCs, ["VQI_SUB_Class4"], 3000]; sleep 1;		// ENEMY SONAR RANGE - 2nd PING - TORP IN WATER!
_ping5 = nearestObjects [posSUBCs, ["VQI_SUB_Class5"], 3000]; sleep 1;		// ENEMY SONAR RANGE - 2nd PING - TORP IN WATER!
_ping6 = nearestObjects [posSUBCs, ["VQI_SUB_Class6"], 7000]; sleep 1;		// ENEMY SONAR RANGE - 2nd PING - TORP IN WATER!
...
HMSProteus = "Submarine_01_F" createVehicle posSUBCs;
...
if (_ENEMY == 0) then {
	_mkrS = createMarker["mkrS", posSUBCs];
	...
	_mkrD = createMarker["mkrD", posSUBCs];
...
if (_ENEMY > 0) then {
	_mkrP1 = createMarker["mkrP1", posSUBCs];
	...
	_mkrP2 = createMarker["mkrP2", posSUBCs];
	...
	_mkrT = createMarker["mkrT", posSUBCs];
	...
	[posSUBCs] execVM "vqi_frogs\VQI-CombatDiver\Navigation\Pinged\EnemyTorp\enemy_torp.sqf";
	subNavBs = [] execVM "vqi_frogs\VQI-CombatDiver\Navigation\Stealth\stealth_wpB.sqf";
};
