// Ballistic Missile Platform --- Scimitar
// Scale DEPTH by Sub Type
if (VQI_FROGS_HINTS == 1) then { hint "sub_class_5.sqf"; };
sleep 1;




private ["_location","_subIntel","_rngSONAR","_rngTRID2","_trgGuided","_subDepth","_subC2bad","_sub""_trgAO","_trgAO1"];
...
// Scimitar
_rngSONAR = 3000;		// 3km
_rngTRID2 = 10000;	// Ballistic Missile
_subDepth = 0;		// surface
_subC2bad = "EAST D";	// Detected by EAST
trgSUB_BM = 1;		// Set Trigger


_sub = "VQI_SUB_Class5" createVehicle _location;
...
_trgAO setTriggerStatements ["({_x in thisList} count (playableUnits + switchableUnits) > 0) && ((getPosASL Player select 2) < -3)",		// SONAR Tech (below water)
						format["[%1] execVM ""vqi_frogs\VQI-CombatDiver\AO\sub\class5\sub_class_5_trg_sonar.sqf""",_subDepth],			// Response Trigger File
						""]; 																								// Exit Trigger Area
sleep 2;


// Ballistic Missiles Trigger
_trgAO1 = createTrigger ["EmptyDetector", _location];						// type, location 										// Location
_trgAO1 setTriggerArea [_rngTRID2, _rngTRID2, 0, false, 1000];				// x, y, angle, rectangle, Z 								// Guided Missile Range
...
_trgAO1 setTriggerStatements ["({_x in thisList} count (playableUnits + switchableUnits) > 0) && ((getPosASL Player select 2) > 2) && (trgSUB_BM == 1)",	// trg Reset ???
						format["[%1] execVM ""vqi_frogs\VQI-CombatDiver\AO\sub\class5\sub_class_5_trg_ballistic.sqf""",_location],		// Response Trigger File
						""]; 																								// Exit Trigger Area
