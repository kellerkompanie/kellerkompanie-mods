// General (older tech) --- Crimson II
// Scale DEPTH by Sub Type
if (VQI_FROGS_HINTS == 1) then { hint "sub_class_2.sqf"; };
sleep 1;
...
_sub = "VQI_SUB_Class2" createVehicle _location;
...
_trgAO setTriggerStatements ["({_x in thisList} count (playableUnits + switchableUnits) > 0) && ((getPosASL Player select 2) < -3)",		// SONAR Tech (below water)
						format["[%1] execVM ""vqi_frogs\VQI-CombatDiver\AO\sub\class2\sub_class_2_trg_sonar.sqf""",_subDepth],			// Response Trigger File
						""]; 																								// Exit Trigger Area
sleep 2;
...
_trgAO1 setTriggerStatements ["({_x in thisList} count (playableUnits + switchableUnits) > 0) && ((getPosASL Player select 2) > 2) && (trgSUB_GM == 1)",	// trg Reset ???
						format["[%1] execVM ""vqi_frogs\VQI-CombatDiver\AO\sub\class2\sub_class_2_trg_guided.sqf""",_location],			// Response Trigger File
						""]; 																								// Exit Trigger Area
