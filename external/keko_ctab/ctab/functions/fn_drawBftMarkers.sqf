...
// ------------------ MEMBERS ------------------
{
    ...
    call {
        ...
        private _assignedTeam = assignedTeam (_x select 0);
        if (_assignedTeam == "") then {
            _assignedTeam = "MAIN";
        };
        private _cTabColorTeamIndex = ["MAIN", "RED", "GREEN", "BLUE", "YELLOW"] find _assignedTeam;
        if (_cTabColorTeamIndex == -1 || _cTabColorTeamIndex > 4) then {
            _cTabColorTeamIndex = 0;
        };
        _teamColor = cTabColorTeam select _cTabColorTeamIndex;
