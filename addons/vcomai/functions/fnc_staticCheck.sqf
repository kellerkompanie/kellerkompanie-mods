...
copyToClipboard str _StaticList;
{
	...
	if (_Foot) then {
		...
		private _keko_vcomai_Indoor = false;
		...
		{
			if (_x isKindof "Building") exitWith {_keko_vcomai_Indoor = true;};
		} foreach _Array;
		
		if !(_keko_vcomai_Indoor) then
		{	
			...
			if !(_HASUAV) then
			{
				...
				if !(_AssembledG isEqualTo "") then
				{
					...
					[_Unit,_CurrentBackPack,_StaticCreated] spawn FUNC(PackStatic);
				};
			}
			else {
				if !(_AssembledG isEqualTo "") then	{
