...
if (count _NEnemyA > 0) then {
	private _clste = [_NEnemyA, _SquadLead, true, "AISpeed"] call FUNC(ClstObj);
	...
	if !(_CST) then	{
		if (behaviour _SquadLead isEqualTo "COMBAT" && {((_SquadGroup getVariable [QGVAR(STM),-60]) +  5) < diag_tickTime}) then {
	...
	if ((_SquadGroup getVariable [QGVAR(STM),-60]) +  5 < diag_tickTime) then {
		_SquadGroup setBehaviourStrong "AWARE";
	};
};