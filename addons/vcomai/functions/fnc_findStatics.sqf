...

{
	if (isNull objectParent _x) then {
		...
		{
			_class = [_currentBackPack] call FUNC(Classname);
			...
			if (!(isNil "_parents")) then {
				if (("StaticWeapon" in _parents) || {("Weapon_Bag_Base" in _parents)}) then {
					private _keko_vcomai_HASUAV = false;
					if (["UAV",_currentBackPack,false] call BIS_fnc_inString) then {_keko_vcomai_HASUAV = true;};
					_staticList pushBack [_x,_currentBackPack,_keko_vcomai_HASUAV];
				};
			};
		};
		//END STATIC WEAPON
		...
		private _mineArray = _x call FUNC(HasMine);
		...
		if (GVAR(ARTYENABLE)) then {_x call FUNC(CheckArty);};
	};
} forEach (units _this);
