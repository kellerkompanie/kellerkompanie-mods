...
[(group _SquadLead),_MedList] call FUNC(MedicalHandler);


private _NearbyCover = [_SquadLead] call FUNC(CoverDetect);
private _NEnemyA = [];
...
if (_NearbyCover isEqualTo []) exitWith {
	{
		if (isNull objectParent _x) then {
			...
			if (random 100 < 50) then {
				...
				if (GVAR(Debug)) then {[_Unit,"LOOKING TO FIRE"] call FUNC(DebugText);};
			};
		};
...
if (count _CoverObjects > 0 && {!(_WPos isEqualTo [0,0,0])}) then {
			...
			if (random 100 < 30) then {
				if (count _CoverHardObjects > 0) then {	
					...
					[_x,_FinalPos] spawn {
						...
						if (GVAR(Debug)) then {
							[_unit,"MOVING TO POSITION - A"] call FUNC(DebugText);
							[_Unit,_Pos,[0.71,0.09,0,1]] spawn FUNC(DebugLine);
						};
					};		
				}
				else {
					...
					[_x,_FinalPos] spawn {
						...
						if (GVAR(Debug)) then 
						{
							[_Unit,"MOVING TO POSITION - B"] call FUNC(DebugText);
							[_Unit,_Pos,[0.71,0.09,0,1]] spawn FUNC(DebugLine);
						};
					};
					/*
					else
					{
						...
						[_x,_Pos] spawn
						{
							...
							if (GVAR(Debug)) then 
							{
								[_unit,"MOVING TO POSITION - C"] call FUNC(DebugText);
								[_Unit,_Pos,[0.71,0.09,0,1]] spawn FUNC(DebugLine);
							};
						};			
				...
				if (count _EnemyArray > 0) then	{
					...
					if (GVAR(Debug)) then {
						[_Unit,"LOOKING TO FIRE - ZD"] call FUNC(DebugText);
						[_Unit,(getpos _TargetE),[0.71,0.09,0,1],true] spawn FUNC(DebugLine);
					};					
				};
			};
		};
		...
		waituntil {
			{
				_x params ["_Unit","_Pos"];
				if (GVAR(Debug)) then {
					[_Unit,(format ["MOVING: %1 M",(_Unit distance2D _Pos)])] call FUNC(DebugText);
				};
				...
				private _EnemyList = _Unit targets [true,1000];
				{
					...
					if (_Cansee > 0 && (_x distance2D _Unit < 100)) exitWith {
						...
						_Unit doTarget _x;						
						if (GVAR(Debug)) then {
							[_Unit, "STOP TO RETURN FIRE"] call FUNC(DebugText);
							[_Unit, (getpos _x), [0.71,0.09,0,1], true] spawn FUNC(DebugLine);
						};
					};
				} foreach _EnemyList;
				if (speed _Unit < 0.1) then {
					...
					_Unit setDestination [_Pos, "FORMATION PLANNED", true];					
					if (GVAR(Debug)) then {
						[_Unit,format ["FORCE MOVE! %1",_Pos]] call FUNC(DebugText);
						[_Unit,_Pos,[0.71,0.09,0,1]] spawn FUNC(DebugLine);
					};
				};
			} foreach _this;
				...
				if (_Unit distance2D _Pos < 2) then {
					_unit spawn {
						...
						if (count _EnemyArray > 0) then	{
							...
							if (GVAR(Debug)) then {
								[_this,"RETURN FIRE - Z"] call FUNC(DebugText);
								[_this,(getpos _TargetE),[0.71,0.09,0,1],true] spawn FUNC(DebugLine);
							};
						};
						if (GVAR(Debug)) then {
							[_this,"SET"] call FUNC(DebugText);
						};
					};
	...
	{
		if (isNull objectParent _x) then {
			if (GVAR(GrenadeChance) > 100) then {
				[_x,false,true] spawn FUNC(ForceGrenadeFire);
			};
			_x forceSpeed -1;
			_x setUnitPos "Auto";
			_x setDestination [_WPos, "FORMATION PLANNED", true];	
			if (GVAR(Debug)) then {
				[_x,"MOVING TO POSITION - D"] call FUNC(DebugText);
			};
			if (random 100 < 50) then {
				private _Unit = _x;
				private _EnemyArray = [];
				...
				if (GVAR(Debug)) then {
					[_Unit,"LOOKING TO FIRE - NO COVER"] call FUNC(DebugText);
				};
			};
		};
	} foreach (units _LGroup);
