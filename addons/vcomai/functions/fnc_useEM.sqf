...
waitUntil {
	if ((random 100 <= GVAR(AI_EM_CHN))) then	{
		{
			private _Unit = _x;
			if (isNull objectParent _Unit) then	{
				private _MP = _Unit getVariable [QGVAR(MVWP),[]];
				if (count _MP > 0 && {!(_Unit getVariable [QGVAR(VAULT),false])}) then {
					private _CP = [_MP,_Unit,true,"EM"] call FUNC(ClstObj);
					...
					if (count _IObjA > 0) then {
						...
						if (!(_IObj isKindOf "Man") && {!(isNull _IObj)} && {_IsCloser distance2D _FinalPoint < _IPos distance2D _FinalPoint} && {count _list isEqualTo 0} && {count _list2 isEqualTo 0}) then {
							...
							if (_Height < 6 && {_CD > 2}) then {
								...
								_Unit setVariable [QGVAR(VAULT),true];
								_Unit domove _IPos;
								_Unit setVariable [QGVAR(TO),diag_tickTime];
								
								waitUntil {
									_Unit distance2D _IPos < 3 || {!(alive _Unit)} || {(diag_tickTime - (_Unit getVariable QGVAR(TO))) > 15}
								};
								
								if ((diag_tickTime - (_Unit getVariable QGVAR(TO))) > 15 || !(alive _Unit)) exitwith {};
								
								waitUntil {
									[_Unit,_IObj, 0] call BIS_fnc_isInFrontOf || {!(alive _Unit)} || {(diag_tickTime - (_Unit getVariable QGVAR(TO))) > 15}
								};
								...
								waitUntil
								{
									...
									sleep 0.1;
									_Unit distance2D _IPos < 1.2 || {!(alive _Unit)} || {(diag_tickTime - (_Unit getVariable QGVAR(TO))) > 15}
								};
								
								if ((diag_tickTime - (_Unit getVariable QGVAR(TO))) > 15 || !(alive _Unit)) exitwith {};
		
								[_Unit,true] spawn FUNC(BabeOver);
								_Unit spawn
								{
									sleep 3;
									waitUntil
									...
									sleep 1;
									_this enableAI "MOVE";
									sleep GVAR(AI_EM_CLDWN);
									_This setVariable [QGVAR(VAULT),false];
								};
							};
						};
