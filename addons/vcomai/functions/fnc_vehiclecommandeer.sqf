...
if (GVAR(ClassSteal)) then {
	...
	//Now let's check for nearby vehicle from 
	{
		if (count _Crewmen > 0) then {
			if (_x iskindof "Tank" && {crew _x isEqualTo []} && {_x distance _Leader < GVAR(AIDISTANCEVEHPATH)} && {locked _x != 2}) then	{
				_this addvehicle _x;
			};				
		};
		
		if (count _Pilots > 0) then	{
			if (_x iskindof "Air" && {crew _x isEqualTo []} && {_x distance _Leader < GVAR(AIDISTANCEVEHPATH)} && {locked _x != 2}) then {
				_this addvehicle _x;
			};			
		};
	
		if (_x iskindof "Car" && {crew _x isEqualTo []} && {_x distance _Leader < GVAR(AIDISTANCEVEHPATH)} && {locked _x != 2}) then {
			_this addvehicle _x;
		};
	} foreach vehicles;
}
	...
	{
		if (_x iskindof "LandVehicle" && {crew _x isEqualTo []} && {_x distance _Leader < GVAR(AIDISTANCEVEHPATH)} && {locked _x != 2}) then {
			_this addvehicle _x;
		};
	} foreach vehicles;
};