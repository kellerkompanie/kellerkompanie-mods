...
/*
	...
	Returns:
		NOTHING
	Example1:  driverArray spawn keko_vcomai_fnc_VehicleDetection;
*/

//This script aims to improve AI driving skills...It's basic. But it works.
{
	private _PredictPos = [_x,3] call FUNC(MovePrediction);
	_PredictPos set [2,0.1];
	if (GVAR(Debug)) then {
		_PredictPos spawn {
			private _arrow = "Sign_Arrow_Cyan_F" createVehicleLocal [0,0,0];
			_arrow setPos _this;
			sleep 2;
	...
	private _NR = [_PredictPos, GVAR(DrivingDist)] call BIS_fnc_nearestRoad;
	if !(isNull _NR) then {
		_PredictPos = getpos _NR;
	};
	
	private _obstacles = nearestObjects [_predictPos, ["Strategic"], GVAR(DrivingDist), true];
	...
	if (count _obstacles > 0) then {
		_x forcespeed 10;
		{
			if !(_x getVariable [QGVAR(AVOID), false] || {_x isKindOf "man"} || {_x isKindOf "Helper_Base_F"} || {_x isKindOf "Logic"}) then {
				// Vcom ignore this object until loop complete
				_x setVariable [QGVAR(AVOID), true];
				...
				{
					if ([_hlprArray, _x, true,"Driving"] call FUNC(ClstObj) distance2D _x > 1) then
					{
						private _hlpObj = "Land_Wrench_F" createVehicleLocal _x;
						_hlpObj setVariable [QGVAR(AVOID), true];
						_hlpObj setPos _x;
				...
				[_hlprArray, _obstacles,_x] spawn {
					...
					} forEach (_this select 0);
					{
						...
						_x setVariable [QGVAR(AVOID), false];
	...
	//Avoid units.
	private _Livingobstacles = _predictPos nearObjects ["MAN", (GVAR(DrivingDist) * 2)];
	private _Unit = _x;
	...
	private _Remove = [];
	{
		...
		if !([(side _Unit),(side _x)] call BIS_fnc_sideIsFriendly) then {
			_Remove pushback _x;
		};
	...
	private _NearestUnit = [_Livingobstacles, _x, true,"Driving"] call FUNC(ClstObj);
