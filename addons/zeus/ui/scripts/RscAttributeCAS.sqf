...
switch _mode do {
    case "onLoad": {
        ...
        if (isNil QGVAR(CASPlaneInfoCache)) then
        {
            GVAR(CASPlaneInfoCache) = [[],[],[],[]];

            {
                ...
                if ((_vehicle isKindOf "Plane") and (getNumber (_planeCfg >> "scope") == 2 or getNumber (_planeCfg >> "scopeCurator") == 2)) then
                {
                    ...
                    };
                    {
                        ...
                        if !(_weapons isEqualTo []) then
                        {
                            private _cas_info_list = GVAR(CASPlaneInfoCache) select _forEachIndex;
        ...
        private _cas_info_list = GVAR(CASPlaneInfoCache) select _weaponTypesID;
        {
    ...
    case "confirmed": {
        ...
        private _plane_info = GVAR(CASPlaneInfoCache) select _weaponTypesID select _planeClassID;
        ...
        _logic setdir (missionnamespace getvariable [QGVAR(CAS_dir), direction _logic]);
        ...
        _arguments remoteExec [QFUNC(moduleCAS_server),2];
