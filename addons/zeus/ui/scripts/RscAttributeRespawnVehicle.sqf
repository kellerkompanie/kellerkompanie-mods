...
switch _mode do {
    case "onLoad": {
        ...
        private _respawnID = _unit getVariable [QGVAR(vehicleRespawnData),-1];
    ...
    case "confirmed": {
        ...
        {
            private _initCode = compile ("(_this select 0) setVariable [""keko_zeus_vehicleRespawnData"", " + str _selected  + ", true]; _this call bis_fnc_curatorrespawn;");
            private _curatorSelected = ["vehicle"] call FUNC(getCuratorSelected);
            {
                ...
                _x setVariable [QGVAR(vehicleRespawnData), _selected, true];
