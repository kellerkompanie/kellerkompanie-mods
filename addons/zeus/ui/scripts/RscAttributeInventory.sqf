...
switch _mode do {
    case "onLoad": {
        if (isNil "RscAttributeInventory_list") then
        {
            ...
            private _magazines = []; //--- Store magazines in an array and mark duplicates, so nthey don't appear in the list of all items
            {
                ...
                if (_addonID < 0) then
                {
                    ...
                    private _weaponTypeID = _types findIf {"Glasses" in _x};
                    {
                        private _glasses = configName _x;
        ...
        private _virtualCargo = [_entity] call FUNC(getVirtualArsenal);
        ...
        _ctrlButtonVA ctrladdeventhandler ["buttonclick", {with uinamespace do {["virtualArsenal", [ctrlparent (_this select 0)], objnull] call RscAttributeInventory}}];
    ...
    case "keyDown":    {
        ...
        switch true do
        {
            ...
            {
                ["virtualArsenal", [ctrlparent _ctrlList], objnull] call RscAttributeInventory;
            };
            ...
            default
            {
                private _letter = [_key] call FUNC(dikToLetter);
                if (_letter != "") then
                {
    ...
    case "confirmed": {
        ...
        private _curatorSelected = ["cargo"] call FUNC(getCuratorSelected);
        ...
        _curatorSelected pushBackUnique (missionNamespace getVariable ["BIS_fnc_initCuratorAttributes_target", objNull]);
        {
            ...
            with missionNamespace do {[_box, uiNamespace getVariable ["RscAttributeInventory_cargoVirtual", []], true] call FUNC(updateVirtualArsenal)};
