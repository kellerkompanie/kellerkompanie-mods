    ...
    case "confirmed": {
        ...
        if (_entity isEqualType []) then
        {
            ...
            private _curatorSelectedWPs = ["wp"] call FUNC(getCuratorSelected);
            {
                ...
                {
                    if (local _group) then {_group setformation _selected} else {[_group,_selected] remoteExec ["setformation", leader _group]};
                };
        ...
        } else
        {
            ...
            private _curatorSelectedGrps = ["group"] call FUNC(getCuratorSelected);
            {
                ...
                if (local _leader) then {_x setformation _selected} else {[_x,_selected] remoteExec ["setformation", _leader]};
