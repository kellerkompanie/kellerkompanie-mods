...
switch (_mode) do
{
    ...
    case "NEW_BUTTON":
    {
        [] spawn
        {
            ...
            {
                closeDialog 0;
                _center_object = ([localize "STR_BASIS_OBJECT"] call FUNC(SelectUnits)) select 0;
                if (isNil "_center_object") exitWith {};
                if (isNull _center_object) exitWith {[localize "STR_NO_OBJECT_SELECTED"] call FUNC(ShowZeusMessage); playSound "FD_Start_F"};

                _objects = [localize "STR_OBJECTS"] call FUNC(SelectUnits);
                if (isNil "_objects") exitWith {};
                if (count _objects == 0) exitWith {[localize "STR_NO_OBJECT_SELECTED"] call FUNC(ShowZeusMessage); playSound "FD_Start_F"};
                ...
                [
                    ...
                    QGVAR(RscDisplayAttributes_createAdvancedComposition)
                ] call FUNC(ShowChooseDialog);

                if (count _dialogResult > 0) then
                {
                    if (_dialogResult select 0 == 0) then
                    {
                        ["ADD",controlNull,[],_dialogResult select 2,_composition_details,_dialogResult select 1] call FUNC(RscDisplayAttributes_manageAdvancedComposition);
                    } else
                    {
                        ["ADD",controlNull,[],_dialogResult select 2,_composition_details] call FUNC(RscDisplayAttributes_manageAdvancedComposition);
                    };
                };
            };
            createDialog QGVAR(composition_Dialog);
            ["LOADED"] call FUNC(RscDisplayAttributes_manageAdvancedComposition);
        };
    };
    case "EDIT_BUTTON":
    {
        [] spawn
        {
            ...
            {
                disableSerialization;
                _tvPath = uiNamespace getVariable [QGVAR(composition_last_choice), []];
                ...
                [
                    ...
                    QGVAR(RscDisplayAttributes_editAdvancedComposition)
                ] call FUNC(ShowChooseDialog);

                if (count _dialogResult == 0) exitWith {};
                if (_dialogResult select 0 == 1) then
                {
                    ["ADD",controlNull,[],_dialogResult select 2,_item_details,_dialogResult select 1] call FUNC(RscDisplayAttributes_manageAdvancedComposition);
                } else
                {
                    ["ADD",controlNull,[],_dialogResult select 2,_item_details] call FUNC(RscDisplayAttributes_manageAdvancedComposition);
                };
                ["REMOVE",controlNull,[],_category_name,_item_name] call FUNC(RscDisplayAttributes_manageAdvancedComposition);
            };
            waitUntil {!dialog};
            createDialog QGVAR(composition_Dialog);
    ...
    case "ADD":
    {
        ...
        } else
        {
            ...
            _category_index = _custom_categories find GVAR(composition_category);
        ...
        profileNamespace setVariable [QGVAR(compositions),_custom_compositions];
