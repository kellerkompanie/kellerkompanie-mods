/*
	Name: keko_ctab_fnc_userMenuSelect

	Author(s):
		Gundy, Riouken

	Description:
		Process user menu select events, initiated by "\x\keko\addons\ctab\shared\cTab_markerMenu_controls.hpp"
	...
	Example:
		[1] call keko_ctab_fnc_userMenuSelect;
*/
...
call {
	...
	if (_type == 1) exitWith {
		...
		[call FUNC(getPlayerEncryptionKey),cTabUserSelIcon] call FUNC(addUserMarker);
	};

	// Lock UAV cam to clicked position
	if (_type == 2) exitWith {
		[cTabUserSelIcon select 0] call FUNC(lockUavCamTo);
	};
