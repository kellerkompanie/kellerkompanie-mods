/*
	Name: keko_ctab_fnc_updateLists
	...
	Example:
		call keko_ctab_fnc_updateLists;
*/
...
*/
{
	if ((_x != cTab_player) && {[_x,["ItemcTab","ItemAndroid","ItemMicroDAGR"]] call FUNC(checkGear)}) then {
		0 = _cTabBFTmembers pushBack [_x,_x call FUNC(getInfMarkerIcon),"",name _x,str([_x] call CBA_fnc_getGroupIndex)];
	};
...
*/
{
	if ((side _x in _validSides) && {count (crew _x) > 0} && {_x != _playerVehicle}) then {
		...
		_iconB = "";
		call {
			if (_x isKindOf "MRAP_01_base_F") exitWith {_iconA = QPATHTOF(img\b_mech_inf_wheeled.paa);};
			if (_x isKindOf "MRAP_02_base_F") exitWith {_iconA = QPATHTOF(img\b_mech_inf_wheeled.paa);};
			if (_x isKindOf "MRAP_03_base_F") exitWith {_iconA = QPATHTOF(img\b_mech_inf_wheeled.paa);};
			if (_x isKindOf "Wheeled_APC_F") exitWith {_iconA = QPATHTOF(img\b_mech_inf_wheeled.paa);};
...
{
	if (side _x in _validSides) then {
		if (headgear _x in cTab_helmetClass_has_HCam || {[_x,["ItemcTabHCam"]] call FUNC(checkGear)}) then {
...
if (count _updateInterface > 0) then {
	[_updateInterface] call FUNC(updateInterface);
};
