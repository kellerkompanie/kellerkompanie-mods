/*
 	Name: keko_ctab_fnc_updateInterface
	...
	(Optional)
 	...
 	Example:
		[[["mapType","SAT"],["mapScaleDsp","4"]]] call keko_ctab_fnc_updateInterface;
*/
...
_mode = [_settings,"mode"] call FUNC(getFromPairs);
if (isNil "_mode") then {
	...
	call {
		...
		if (_x select 0 == "nightMode") exitWith {
			...
			_background = call {
				if (_displayName in ["cTab_TAD_dsp","cTab_TAD_dlg"]) exitWith {
					if (_nightMode) then {QPATHTOF(img\TAD_background_night_ca.paa)} else {QPATHTOF(img\TAD_background_ca.paa)};
				};
				if (_displayName in ["cTab_Android_dsp","cTab_Android_dlg"]) exitWith {
					if (_nightMode) then {QPATHTOF(img\android_background_night_ca.paa)} else {QPATHTOF(img\android_background_ca.paa)};
				};
				if (_displayName in ["cTab_microDAGR_dsp","cTab_microDAGR_dlg"]) exitWith {
					if (_nightMode) then {QPATHTOF(img\microDAGR_background_night_ca.paa)} else {QPATHTOF(img\microDAGR_background_ca.paa)};
				};
				if (_displayName in ["cTab_Tablet_dlg"]) exitWith {
					if (_nightMode) then {QPATHTOF(img\tablet_background_night_ca.paa)} else {QPATHTOF(img\tablet_background_ca.paa)};
				};
				""
			};
			if (_background != "") then {
				...
				if (!_interfaceInit) then {
					_settings pushBack ["brightness",[_displayName,"brightness"] call FUNC(getSettings)];
				};
			};
		};
			...
			if !(_displayItems isEqualTo []) then {
				...
				call {
					...
					if (_mode == "BFT") exitWith {
						...
						_mapIDC = [_mapTypes,_mapType] call FUNC(getFromPairs);
						...
						if (!_interfaceInit) then {
							if (_isDialog) then {
								_settings pushBack ["mapScaleDlg",[_displayName,"mapScaleDlg"] call FUNC(getSettings)];
								_settings pushBack ["mapWorldPos",[_displayName,"mapWorldPos"] call FUNC(getSettings)];
							};
						};
					};
					// ---------- _NOT_ BFT -----------
					if (_isDialog) then {
						...
						if (count _mapTypes > 1) then {
							...
							_targetMapIDC = [_mapTypes,_targetMapName] call FUNC(getFromPairs);
							...
							if (ctrlShown _targetMapCtrl) then {
								...
								[_displayName,[["mapWorldPos",cTabMapWorldPos],["mapScaleDlg",_mapScale]],false] call FUNC(setSettings);
							};
						};
					};
					// ---------- UAV -----------
					if (_mode == "UAV") exitWith {
						...
						if (!_interfaceInit) then {
							_settings pushBack ["uavCam",[_displayName,"uavCam"] call FUNC(getSettings)];
						};
					};
					// ---------- HELMET CAM -----------
					if (_mode == "HCAM") exitWith {
						...
						if (!_interfaceInit) then {
							_settings pushBack ["hCam",[_displayName,"hCam"] call FUNC(getSettings)];
						};
					};
					// ---------- MESSAGING -----------
		...
		if (_x select 0 == "uavCam") exitWith {
			if (_mode == "UAV") then {
				_data = _x select 1;
				if (_data != "") then {
					[_data,[[0,"rendertarget8"],[1,"rendertarget9"]]] spawn FUNC(createUavCam);
				} else {
		...
		if (_x select 0 == "hCam") exitWith {
			...
			if (!isNil "_renderTarget") then {
				_data = _x select 1;
				if (_data != "") then {
					[_renderTarget,_data] spawn FUNC(createHelmetCam);
				} else {
