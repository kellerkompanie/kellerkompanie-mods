/*
 	Name: keko_ctab_fnc_setSettings
	...
	Parameters:
 	...
 	Example:
		["cTab_Tablet_dlg",[["mapType","SAT"],["mapScaleDsp","4"]]] call keko_ctab_fnc_setSettings;

		// Update mapWorldPos and update the interface even if the value has not changed
		["cTab_Tablet_dlg",[["mapWorldPos",getPosASL vehicle player]],true,true] call keko_ctab_fnc_setSettings;

		// Update mapWorldPos and mapScale, but do not update the interface
		["cTab_Tablet_dlg",[["mapWorldPos",getPosASL vehicle player],["mapScaleDsp","2"]],false] call keko_ctab_fnc_setSettings;
*/
...
_combinedPropertiesUpdate = [];
{
	...
	_value = _x select 1;
	call {
		_currentValue = [_groupProperties,_key] call FUNC(getFromPairs);
		...
		_currentValue = [_commonProperties,_key] call FUNC(getFromPairs);
...
if (!isNil "cTabIfOpen") then {
	call {
		...
		if ((([cTabDisplayPropertyGroups,cTabIfOpen select 1] call FUNC(getFromPairs)) == _propertyGroupName) && {count _combinedPropertiesUpdate > 0}) exitWith {
