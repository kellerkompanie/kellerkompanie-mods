/*
	...
	Example:
		...
		[0,"cTab_TAD_dsp",player,vehicle player] call keko_ctab_fnc_open;
*/
...
cTabIfOpen = [_interfaceType,_displayName,_player,
	_player addEventHandler ["killed",{[] call FUNC(close)}],
	_vehicle,nil,nil,nil,nil];

if (_vehicle != _player && (_isDialog || _displayName in ["cTab_TAD_dsp"])) then {
	cTabIfOpen set [5,
		_vehicle addEventHandler ["GetOut",{if (_this select 2 == cTab_player) then {[] call FUNC(close)}}]
	];
};
...
if (_displayName in ["cTab_TAD_dsp","cTab_TAD_dlg"]) then {
	cTabIfOpen set [6,
		addMissionEventHandler ["Draw3D",{
			...
			// update time
			(_display displayCtrl IDC_CTAB_OSD_TIME) ctrlSetText call FUNC(currentTime);
	...
	cTabIfOpen set [6,
		addMissionEventHandler ["Draw3D",{
			...
			(_display displayCtrl IDC_CTAB_OSD_DIR_OCTANT) ctrlSetText format ["%1",[_heading] call FUNC(degreeToOctant)];
		}]
	];
};
