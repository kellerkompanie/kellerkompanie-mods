/*
 	Name: keko_ctab_fnc_createHelmetCam
	...
	Parameters:
 	...
 	Example:
		["rendertarget12",str player] spawn keko_ctab_fnc_createHelmetCam;
*/
...
if (IsNull _newHost) exitWith {
	["cTab_Tablet_dlg",[["mode","HCAM"]]] call FUNC(setSettings);
	false
};
...
if (!isNil "cTabHcams") then {
	...
	if (_oldHost isEqualTo _newHost) then {
		...
		_nop = [] call FUNC(deleteHelmetCam);
		waitUntil {_nop};
	};
};
