/*
	Name: keko_ctab_fnc_getUserMarkerList
	...
	Example:
		...
		[player] call keko_ctab_fnc_getUserMarkerList;

		// Client receiving the list from the server
		[[userMarkerList],157] call keko_ctab_fnc_getUserMarkerList;
*/
...
call {
	...
	if (count _this == 0) exitWith {
		if (hasInterface && !isServer) then {
			[[player],QFUNC(getUserMarkerList),false,false,true] call bis_fnc_MP;
		};
	};
	...
	if (count _this == 1) exitWith {
		if (isServer && !hasInterface && (cTab_userMarkerTransactionId >= 0)) then {
			[[cTab_userMarkerLists,cTab_userMarkerTransactionId],QFUNC(getUserMarkerList),_this select 0,false,true] call bis_fnc_MP;
		};
	};
