/*
	...
	Example:
		...
		["bluefor",[[1714.35,5716.82],0,0,0,"12:00",player]]call keko_ctab_fnc_addUserMarker;

		// Client receiving marker addition (from server)
		["bluefor",[21,[[1714.35,5716.82],0,0,0,"12:00",player]],157]call keko_ctab_fnc_addUserMarker;
*/
...
call {
	...
	if (isServer) exitWith {
		if (isNil "_transactionId") then {
			...
			[cTab_userMarkerLists,_encryptionKey,[[_transactionId,_markerData]]] call FUNC(addToPairs);

			// Send addUserMarker command to all clients
			[[_encryptionKey,[_transactionId,_markerData],_transactionId],QFUNC(addUserMarker),true,false,true] call bis_fnc_MP;
	...
	if (hasInterface && !isNil "_transactionId") exitWith {
		call {
			...
			[cTab_userMarkerLists,_encryptionKey,[_markerData]] call FUNC(addToPairs);
	...
	if (hasInterface) then {
		[_this,QFUNC(addUserMarker),false,false,true] call bis_fnc_MP;
	};
};
