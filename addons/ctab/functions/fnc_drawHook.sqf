/*
	...
	Parameters:
 	...
 	Example:
		[_display,_ctrlScreen,_playerPos,cTabMapCursorPos,0,false] call keko_ctab_fnc_drawHook;
*/
...
_dirToSecondPos = call {
	if (_this select 4 == 0) exitWith {
		...
		[_pos,_secondPos] call FUNC(dirTo)
	};
	_ctrlScreen drawArrow [_secondPos,_pos,cTabMicroDAGRhighlightColour];
	[_secondPos,_pos] call FUNC(dirTo)
};
_dstToSecondPos = [_pos,_secondPos] call FUNC(distance2D);
call {
	...
	(_display displayCtrl IDC_CTAB_OSD_HOOK_DIR) ctrlSetText format ["%1° %2",[_dirToSecondPos,3] call CBA_fnc_formatNumber,[_dirToSecondPos] call FUNC(degreeToOctant)];
