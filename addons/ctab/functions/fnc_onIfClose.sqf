...
if !(isNil "cTabIfOpen") then {
	...
	if (!cTabIfOpenStart) then {
		if ([_displayName] call FUNC(isDialog)) then {
			...
			[_displayName,[["mapWorldPos",cTabMapWorldPos],["mapScaleDlg",_mapScale],["dlgIfPosition",_backgroundOffset]],false] call FUNC(setSettings);
		};
	};
