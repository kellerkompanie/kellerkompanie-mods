...
#include "\x\keko\addons\ctab\functions\keys.sqf"
...
["cTab_updatePulse",FUNC(updateLists)] call CBA_fnc_addEventHandler;
...
cTab_fnc_onIfMainPressed = {
	...
	_interfaceName = call {
		if ([_player,_vehicle,"TAD"] call FUNC(unitInEnabledVehicleSeat)) exitWith {
			cTabPlayerVehicleIcon = getText (configFile/"CfgVehicles"/typeOf _vehicle/"Icon");
			"cTab_TAD_dsp"
		};
		if ([_player,["ItemAndroid"]] call FUNC(checkGear)) exitWith {"cTab_Android_dsp"};
		if ([_player,["ItemMicroDAGR"]] call FUNC(checkGear)) exitWith {
			cTabMicroDAGRmode = if ([_player,["ItemcTab"]] call FUNC(checkGear)) then {0} else {2};
			"cTab_microDAGR_dsp"
		};
		if ([_player,_vehicle,"FBCB2"] call FUNC(unitInEnabledVehicleSeat)) exitWith {"cTab_FBCB2_dlg"};
		if ([_player,["ItemcTab"]] call FUNC(checkGear)) exitWith {"cTab_Tablet_dlg"};
		// default
		""
	};
...
cTab_fnc_onIfSecondaryPressed = {
	...
	_interfaceName = call {
		...
		if ([_player,_vehicle,"FBCB2"] call FUNC(unitInEnabledVehicleSeat)) exitWith {"cTab_FBCB2_dlg"};
		if ([_player,["ItemAndroid"]] call FUNC(checkGear)) exitWith {"cTab_Android_dlg"};
		if ([_player,["ItemMicroDAGR"]] call FUNC(checkGear)) exitWith {
			cTabMicroDAGRmode = if ([_player,["ItemcTab"]] call FUNC(checkGear)) then {0} else {2};
			"cTab_microDAGR_dlg"
		};
...
cTab_fnc_iconText_toggle = {
	...
	[_displayName,[["showIconText",cTabBFTtxt]]] call FUNC(setSettings);
	true
};
...
cTabOnDrawbft = {
	...
	cTabMapWorldPos = [_cntrlScreen] call FUNC(ctrlMapCenter);
	cTabMapScale = ctrlMapScale _cntrlScreen;

	[_cntrlScreen,true] call FUNC(drawUserMarkers);
	[_cntrlScreen,0] call FUNC(drawBftMarkers);
	...
	if (cTabDrawMapTools) then {
		[_display,_cntrlScreen,_playerPos,cTabMapCursorPos,0,false] call FUNC(drawHook);
	};
...
cTabOnDrawbftTAD = {
	...
	[_cntrlScreen,false] call FUNC(drawUserMarkers);
	[_cntrlScreen,1] call FUNC(drawBftMarkers);
	...
	_cntrlScreen drawIcon [QPATHTOF(img\TAD_overlay_ca.paa),cTabTADfontColour,_playerPos,250,250,_heading,"",1,cTabTxtSize,"TahomaB","right"];
...
cTabOnDrawbftTADdialog = {
	...
	call {
		if (cTabDrawMapTools) exitWith {
			[_display,_cntrlScreen,_playerPos,cTabMapCursorPos,0,true] call FUNC(drawHook);
		};
		[_display,_cntrlScreen,_playerPos,cTabMapCursorPos,1,true] call FUNC(drawHook);
	};
	true
};
...
cTabOnDrawbftmicroDAGRdsp = {
	...
	[_cntrlScreen,false] call FUNC(drawUserMarkers);
	[_cntrlScreen,cTabMicroDAGRmode] call FUNC(drawBftMarkers);
...
[] spawn {
	...
	["cTab","toggleIfPosition",["Toggle Interface Position","Toggle overlay mode interface position from left to right or reset interactive mode interface position to default"],{[] call FUNC(toggleIfPosition)},"",[cTab_key_toggleIfPosition_scancode,cTab_key_toggleIfPosition_modifiers],false] call cba_fnc_addKeybind;
	...
	if (player in (call BIS_fnc_listCuratorPlayers)) then {
		[] spawn {
			while {true} do {
				...
				(findDisplay 312) displayAddEventHandler ["KeyDown","[_this,'keydown'] call keko_ctab_fnc_processCuratorKey"];
				(findDisplay 312) displayAddEventHandler ["KeyUp","[_this,'keyup'] call keko_ctab_fnc_processCuratorKey"];
				waitUntil {sleep 0.1;isNull (findDisplay 312)};
			};
		};
	};
...
cTab_msg_gui_load = {
	...
	// Messages
	{
		...
		_img = call {
			if (_msgState == 0) exitWith {QPATHTOF(img\icoUnopenedmail.paa)};
			if (_msgState == 1) exitWith {QPATHTOF(img\icoOpenmail.paa)};
			if (_msgState == 2) exitWith {QPATHTOF(img\icon_sentMail_ca.paa)};
		};
	...
	{
		if ((side _x in _validSides) && {isPlayer _x} && {[_x,["ItemcTab","ItemAndroid"]] call FUNC(checkGear)}) then {
...
cTab_msg_Send = {
	...
	if (_recipientNames != "") then {
		...
		if (!isNil "cTabIfOpen" && {[cTabIfOpen select 1,"mode"] call FUNC(getSettings) == "MESSAGE"}) then {
...
["cTab_msg_receive",
	{
		...
		if (_msgRecipient == cTab_player && _sender != cTab_player && {_playerEncryptionKey == _msgEncryptionKey} && {[cTab_player,["ItemcTab","ItemAndroid"]] call FUNC(checkGear)}) then {
			...
			if (!isNil "cTabIfOpen" && {[cTabIfOpen select 1,"mode"] call FUNC(getSettings) == "MESSAGE"}) then {
				...
				// add a notification
				["MSG",format ["New message from %1",name _sender],6] call FUNC(addNotification);
			} else {
...
*/
cTab_Tablet_btnACT = {
	_mode = ["cTab_Tablet_dlg","mode"] call FUNC(getSettings);
	call {
		if (_mode == "UAV") exitWith {[] call FUNC(remoteControlUav);};
		if (_mode == "HCAM") exitWith {["cTab_Tablet_dlg",[["mode","HCAM_FULL"]]] call FUNC(setSettings);};
		if (_mode == "HCAM_FULL") exitWith {["cTab_Tablet_dlg",[["mode","HCAM"]]] call FUNC(setSettings);};
	};
	true
};
