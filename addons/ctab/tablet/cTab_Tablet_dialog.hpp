...
#include "\x\keko\addons\ctab\shared\cTab_markerMenu_macros.hpp"

class cTab_Tablet_dlg {
	idd = 1775154;
	movingEnable = true;
	onLoad = "_this call keko_ctab_fnc_onIfOpen;";
	onUnload = "[] call keko_ctab_fnc_onIfclose;";
	onKeyDown = "_this call keko_ctab_fnc_onIfKeyDown;";
	objects[] = {};
	class controlsBackground {
		...
		class windowsBar: cTab_RscPicture
		{
			...
			text = QPATHTOF(img\Desktop_bar.jpg);
		...
		class screen: cTab_Tablet_RscMapControl
		{
			...
			onMouseButtonDblClick = "_ok = [3300,_this] execVM '\x\keko\addons\ctab\shared\cTab_markerMenu_load.sqf';";
	...
	class controls {
		...
		class Desktop: cTab_RscControlsGroup
		{
			...
			class controls
			{
				class actBFTtxt: cTab_ActiveText
				{
					...
					text = QPATHTOF(img\cTab_BFT_ico.paa) ;//"Blue Force Tracker"; //--- ToDo: Localize;
		...
		class UAV: cTab_RscControlsGroup
		{
			...
			class controls
			{
				...
				class cTabUAVlist: cTab_RscListbox_Tablet
				{
					...
					w = pxToScreen_W(cTab_GUI_tablet_WINDOW_CONTENT_W);
					h = pxToScreen_H(cTab_GUI_tablet_WINDOW_CONTENT_H);
					onLBSelChanged = "if (!cTabIfOpenStart && (_this select 1 != -1)) then {['cTab_Tablet_dlg',[['uavCam',(_this select 0) lbData (_this select 1)]]] call keko_ctab_fnc_setSettings;};";
				};
		...
		class HCAM: cTab_RscControlsGroup
		{
			...
			class controls
			{
				...
				class cTabHcamList: cTab_RscListbox_Tablet
				{
					...
					w = pxToScreen_W(cTab_GUI_tablet_WINDOW_CONTENT_W);
					h = pxToScreen_H(cTab_GUI_tablet_WINDOW_CONTENT_H);
					onLBSelChanged = "if (!cTabIfOpenStart && (_this select 1 != -1)) then {['cTab_Tablet_dlg',[['hCam',(_this select 0) lbData (_this select 1)]]] call keko_ctab_fnc_setSettings;};";
				};
		...
		*/
		...
		#include "\x\keko\addons\ctab\shared\cTab_markerMenu_controls.hpp"
		...
		*/
		...
		class btnF5: cTab_Tablet_btnF5
		{
			...
			action = "['cTab_Tablet_dlg'] call keko_ctab_fnc_toggleMapTools;";
		};
		...
		{
			IDC_COUNTER
			action = "['cTab_Tablet_dlg'] call keko_ctab_fnc_centerMapOnPlayerPosition;";
		...
		class btnMain: cTab_Tablet_btnHome
		{
			...
			tooltip = "Main Menu";
			action = "['cTab_Tablet_dlg',[['mode','DESKTOP']]] call keko_ctab_fnc_setSettings;";
		};
