	...
	class controls
	{
		...
		*/
		class pwrbtn: cTab_RscButton_TAD_DNO
		{
			...
			onMouseButtonUp = "if (_this select 1 == 0) then {['cTab_TAD_dlg'] call cTab_fnc_toggleNightMode} else {if (_this select 1 == 1) then {[] call keko_ctab_fnc_close};}";
		...
		{
			IDC_COUNTER
			action = "['cTab_TAD_dlg'] call keko_ctab_fnc_toggleMapTools;";
			tooltip = "Toggle Map Tools (F5)";
		};
		class btnF7: cTab_RscButton_TAD_OSB19
		{
			IDC_COUNTER
			action = "['cTab_TAD_dlg'] call keko_ctab_fnc_centerMapOnPlayerPosition;";
