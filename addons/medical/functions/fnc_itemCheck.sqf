...
if ( _isMedic > 0) then {
	while {({_x == "Medikit"} count items _unit) > 0} do {
		...
		[_unit, "ACE_fieldDressing", 20] call _add;
		[_unit, "ACE_elasticBandage", 50] call _add;
		[_unit, "ACE_epinephrine", 8] call _add;
		[_unit, "ACE_morphine", 8] call _add;
		[_unit, "ACE_tourniquet", 4] call _add;
		[_unit, "ACE_salineIV", 6] call _add;
		[_unit, "ACE_salineIV_500", 6] call _add;
		[_unit, "KAT_Painkillers", 1] call _add;
		[_unit, "ACE_surgicalKit", 1] call _add;

		if ( _isMedic == 1) then {
			[_unit, "KAT_Pulseoximeter", 1] call _add;
		} else {
			[_unit, "KAT_X_AED", 1] call _add;
		};
		...
		if ( _airway ) then {
			if ( _isMedic == 1) then {
				[_unit, "KAT_guedel", 10] call _add;
			} else {
				[_unit, "KAT_larynx", 10] call _add;
				[_unit, "KAT_accuvac", 1] call _add;
			};
		};
	};
};
