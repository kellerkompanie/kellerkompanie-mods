...
if(count _syncedObjects == 1) then {
    ...
    if(_syncedObject isKindOf QGVAR(modulePOI3den)) then {
        ...
        private _markerName = _poi_id + "_spawn";
        private _marker = createMarker [_markerName, getPos _logic];
        _marker setMarkerAlpha 0;
    }
    else {
        systemChat "ERROR: Spawn Point Module needs to be synchronized with POI module";
    };
} else {
        systemChat "ERROR: Spawn Point Module cannot be synchronized with more than 1 object";
};
